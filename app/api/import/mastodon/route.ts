...
import { fetchMastodonPublicArchive, isValidMastodonHandle } from "../../../../lib/mastodon-shared";
...
export async function POST(request: Request) {
  ...
  if (!isValidMastodonHandle(handle)) {
  ...
  if (!isValidMastodonHandle(handle)) {
    return NextResponse.json({ error: "Enter a valid Mastodon handle like @you@mastodon.social." }, { status: 400 });
  }

  const { error: accountError } = await supabase.from("connected_accounts").upsert(
    { user_id: userId, platform: "mastodon", platform_username: handle, status: "importing", metadata: { mode: "public" } },
  ...
  const { data: job } = await supabase
    .from("import_jobs")
    .insert({ user_id: userId, platform: "mastodon", status: "running", started_at: new Date().toISOString() })
    .select("id")
    .single();

  try {
    const items = await fetchMastodonPublicArchive(handle);
    const imported = await upsertArchiveItems(supabase, userId, "mastodon", items);
