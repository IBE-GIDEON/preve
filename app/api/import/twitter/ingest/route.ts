...
// Heavy X users have a lot of tweets; cap per request, chunked from the client.
...
export async function POST(request: Request) {
  ...
  const displayName =
    typeof body.name === "string" && body.name.trim() ? body.name.trim().slice(0, 120) : "X archive";

  const now = new Date().toISOString();
  try {
    const imported = await upsertArchiveItems(supabase, userId, "x", items);
    ...
    await supabase.from("connected_accounts").upsert(
      {
        ...
        status: "connected",
        last_sync_at: now,
        metadata: { import_source: "data_archive" },
      },
